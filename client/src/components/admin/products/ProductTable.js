...
const ProductTable = ({ product, deleteProduct, editProduct }) => {
  // Compute expiration using expiryAt set by backend, fallback to createdAt + pTimeWindowHours
  const createdAt = moment(product.createdAt);
  const expiryAt = product.expiryAt ? moment(product.expiryAt) : createdAt.clone().add(Number(product.pTimeWindowHours) || 0, "hours");
  const isExpired = expiryAt.isBefore(moment());
  const expirationTimeString = expiryAt.format("LLL");
  const visibleStatus = isExpired ? "Disabled" : product.pStatus;
